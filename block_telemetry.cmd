Rem This script must be run by an administrator or shift-right click-run as administrator
Rem Microsoft will always send telemetry data home to Microsoft even if you turned all the
Rem telemetry options off, and even if you do not sign into a Microsoft account.
Rem Protect your data, and above all stay safe.

Rem Blocking all Microsoft Servers at the firewall level
...
netsh advfirewall firewall add rule name="telemetry_telecommand.telemetry.microsoft.com.nsatc.net" dir=out action=block remoteip=65.55.252.92 enable=yes
...
netsh advfirewall firewall add rule name="telemetry_sqm.telemetry.microsoft.com.nsatc.net" dir=out action=block remoteip=65.55.252.93 enable=yes
...
netsh advfirewall firewall add rule name="telemetry_watson.telemetry.microsoft.com.nsatc.net" dir=out action=block remoteip=65.55.252.43,65.52.108.29 enable=yes
...
netsh advfirewall firewall add rule name="telemetry_pre.footprintpredict.com" dir=out action=block remoteip=204.79.197.200 enable=yes
netsh advfirewall firewall add rule name="telemetry_i1.services.social.microsoft.com" dir=out action=block remoteip=104.82.22.249 enable=yes
netsh advfirewall firewall add rule name="telemetry_i1.services.social.microsoft.com.nsatc.net" dir=out action=block remoteip=104.82.22.249 enable=yes
...
netsh advfirewall firewall add rule name="telemetry_msnbot-65-39-117-230.search.msn.com" dir=out action=block remoteip=65.39.117.230 enable=yes
...
netsh advfirewall firewall add rule name="telemetry_a134-170-30-202.deploy.static.akamaitechnologies.com" dir=out action=block remoteip=134.170.30.202 enable=yes
netsh advfirewall firewall add rule name="telemetry_a137-116-81-24.deploy.static.akamaitechnologies.com" dir=out action=block remoteip=137.116.81.24 enable=yes
netsh advfirewall firewall add rule name="telemetry_vortex-win.data.microsoft.com" dir=out action=block remoteip=20.50.201.195 enable=yes
netsh advfirewall firewall add rule name="telemetry_onedscolprdweu01.westeurope.cloudapp.azure.com" dir=out action=block remoteip=20.50.201.195 enable=yes

Rem Disabling the services that call out to Microsoft
sc config "diagsvc" start= disabled &:: Diagnostic Execution Service
sc config "DPS" start= disabled &:: Diagnostic Policy Service
sc config "WdiServiceHost" start= disabled &:: Diagnostic Service Host
sc config "WdiSystemHost" start= disabled &:: Diagnostic System Host
sc config "dmwappushsvc" start= disabled &::
sc config "DiagTrack" start= disabled &:: Diagnostic Tracking Service

Rem comments (REM, ::) only work at the beginning of a line
Rem so to get around that we use the command concatenation character (&)
Rem like &:: or & REM . Keep in mind double colon comments doesn't seem
Rem to work inside code blocks (code between parentheses, like in if/for loops).